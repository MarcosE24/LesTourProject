...
class HotelesTest(TestCase):
    ...
    def test_hoteles_contenido(self): #testea si la vista muestra contenido correcto
        url = reverse('hoteles')
        respuesta = self.client.get(url)
        self.assertContains(respuesta, "¡Hola, mundo!")
...
class UsuarioTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        Usuario.objects.create(nombre='Marcos Escobar', edad=25, telefono=986543682)

    def test_nombre_max_length(self):
        modelo = Usuario.objects.get(id=1)
        max_length = modelo._meta.get_field('nombre').max_length
        self.assertEquals(max_length, 100)

    def test_str_method(self):
        modelo = Usuario.objects.get(id=1)
        self.assertEquals(str(modelo), 'Ejemplo')
